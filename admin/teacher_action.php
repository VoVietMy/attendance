...
if (isset ($_POST["action"])) {
	if ($_POST["action"] == "fetch") {
		...
		foreach ($result as $row) {
			...
			$sub_array[] = '<button type="button" name="view_teacher" class="btn btn-info btn-sm 	_teacher" id="' . $row["teacher_id"] . '">View</button>';
	...
	if ($_POST["action"] == 'Add' || $_POST["action"] == "Edit") {
		...
		if (empty ($_POST["teacher_name"])) {
			$error_teacher_name = 'Chưa có tên giáo viên';
			$error++;
		} else {
			$teacher_name = $_POST["teacher_name"];
		}
		if (empty ($_POST["teacher_address"])) {
			$error_teacher_address = 'Chưa có địa chỉ';
			$error++;
		} else {
