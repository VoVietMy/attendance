...
if (isset($_POST["action"])) {
	...
	if ($_POST["action"] == 'Add' || $_POST["action"] == "Edit") {
		...
		if ($error > 0) {
			...
			if ($_POST["action"] == "Edit") {	// Lấy student_roll_number từ dữ liệu gửi lên
				$student_roll_number = $_POST["student_roll_number"];

				// Kiểm tra xem student_roll_number đã tồn tại trong cơ sở dữ liệu chưa
				$query_check_roll_number = "
					SELECT * FROM tbl_student 
					WHERE student_roll_number = :student_roll_number
				";
				$statement_check_roll_number = $connect->prepare($query_check_roll_number);
				$statement_check_roll_number->bindParam(':student_roll_number', $student_roll_number, PDO::PARAM_STR);
				$statement_check_roll_number->execute();
				$count = $statement_check_roll_number->rowCount();

				if ($count > 0) {
					// Nếu student_roll_number đã tồn tại, trả về thông báo lỗi
					$output = array(
